...
def eval_intrinsics_lookup(TOF_H_INV: np.ndarray, orig_res: tuple, target_res: tuple):
...
def process_points(pcl_z, xu_lookup_tof, yv_lookup_tof, TOF_RES, TOF_RES_SCALED, PBIN, ZCUTOFF, TF):
    # downsizing Z only ================================================================================================
...
class NodeDepthMap:
    ...
    def _process_pointcloud(self):
        ...
        if self.do_init_flg is True:
            ...
            self.TOF_XU_LOOKUP         = eval_intrinsics_lookup(self.TOF_H_INV, self.TOF_RES, self.TOF_RES_SCALED)[0]
            self.TOF_YV_LOOKUP         = eval_intrinsics_lookup(self.TOF_H_INV, self.TOF_RES, self.TOF_RES_SCALED)[1]

        # do the processing --------------------------------------------------------------------------------------------
        depth_map, xu_lookup_map, yv_lookup_map = process_points(
